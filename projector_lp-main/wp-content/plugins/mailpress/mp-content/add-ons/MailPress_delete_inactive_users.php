<?php
if ( class_exists( 'MailPress' ) && !class_exists( 'MailPress_delete_inactive_users' ) )
{
/*
Plugin Name: MailPress_delete_inactive_users
Plugin URI: http://blog.mailpress.org/tutorials/add-ons/delete_inactive_users/
Description: Users : delete inactive/bounced/unsubscribed mp users
Version: 7.2
*/

class MailPress_delete_inactive_users
{
	const option_name = 'MailPress_delete_inactive_users';

	const process_name		= 'mp_process_delete_inactive_users';
	...
	function __construct()
	{
		...
		{	
			add_action( 'MailPress_schedule_delete_inactive_users', 	array( __CLASS__, 'schedule' ) );
		}
...
// process
	public static function process()
	{
		...
		$ids = $wpdb->get_results( $wpdb->prepare( "SELECT id FROM $wpdb->mp_users WHERE status in ( 'waiting', 'inactive', 'unsubscribed' ) AND DATE( laststatus ) < %s;", $date ) );

		foreach( $ids as $id ) MP_User::delete( $id->id );
	}
...
// for install
	public static function install() 
	{
		...
		do_action( 'MailPress_schedule_delete_inactive_users' );
	}
...
// for settings batches
	...
	{
		include ( MP_ABSPATH . 'mp-admin/includes/settings/batches/delete_inactive_users/help.php' );
		return $content;
	}

	public static function settings_batches_update()
	{
		include ( MP_ABSPATH . 'mp-admin/includes/settings/batches/delete_inactive_users/update.php' );
	}

	public static function settings_batches_form()
	{
		include ( MP_ABSPATH . 'mp-admin/includes/settings/batches/delete_inactive_users/form.php' );
	}

}
new MailPress_delete_inactive_users();
}