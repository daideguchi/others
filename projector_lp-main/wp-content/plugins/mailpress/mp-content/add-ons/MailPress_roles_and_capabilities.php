...
class MailPress_roles_and_capabilities
{
	...
	// for settings
		...
		add_filter( 'MailPress_settings_tab', 		array( __CLASS__, 'settings_tab' ), 100, 1 );
	//for ajax
...
// for settings
	...
	{
		$rcs_option = 'MailPress_r&c_' . MP_WP_Ajax::$pst_['role'];
		$r = get_role( MP_WP_Ajax::$pst_['role'] );

		$rcs = get_option( $rcs_option );
		if ( empty( $rcs ) ) $rcs = array();

		if ( MP_WP_Ajax::$pst_['add'] )
		{
			$rcs[MP_WP_Ajax::$pst_['capability']] = 'on';
			if ( $r ) $r->add_cap( MP_WP_Ajax::$pst_['capability'] );
		}
		else
		{
			unset ( $rcs[MP_WP_Ajax::$pst_['capability']] );
			if ( $r ) $r->remove_cap( MP_WP_Ajax::$pst_['capability'] );
		}
