...
//// WP user management ////
	...
	public static function profile_update( $wid )
	{
		...
		if ( $id )
		{
			...
			if ( stripslashes( $post_['display_name'] ) != $mp_user->name )
			{
				MP_User::update_name( $id, $post_['display_name'] );
			}
			if ( $post_['email'] != $mp_user->email )
			{
				$wp_user = self::get_wp_user( $wid );
				if ( MailPress::is_email( $post_['email'] ) ) self::sync( $wp_user );
			}
		}
		else
		{
