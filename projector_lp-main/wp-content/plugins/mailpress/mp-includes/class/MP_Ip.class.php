...
class MP_Ip
{
	...
	public static function get_current_latlng()
	{
		$ip = filter_input( INPUT_SERVER, 'REMOTE_ADDR' );
		return self::get_latlng( $ip );
	}
	...
	public static function get_context( $pdata, $ip )
	{
		...
		$cache = MP_UPL_ABSPATH . 'ip/' . ( ( !isset( $pdata['md5'] ) ) ?  md5( $ip_url ) : $ip_url ) . '.spc';

		if ( is_file( $cache ) )
		{
			$cache_days = ( $pdata['cache_days'] ?? self::cache_days ) *24*60*60;
	...
	public static function get_address( $lat, $lng )
	{
		...
		return MP_Map::get_address( $lng, $lat );
	}
}