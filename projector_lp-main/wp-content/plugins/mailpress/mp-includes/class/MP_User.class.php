...
class MP_User
{
	...
	public static function update_status( $id, $status ) 
	{
		...
		$data['laststatus_IP'] 	= ( isset( $REMOTE_ADDR ) )     ? $REMOTE_ADDR : '';				   		$format[] = '%s';
		$data['laststatus_agent'] 	= ( isset( $HTTP_USER_AGENT ) ) ? trim( strip_tags( $HTTP_USER_AGENT ) ) : ''; 	$format[] = '%s';
	...
	public static function unsubscribed( $id, $from_list = false ) 
	{
		...
		if ( $update ) 
		{
			if ( !$from_list ) do_action( 'MailPress_unsubscribe_user', $id );
			new MP_Stat( 'u', 'waiting', -1 );
			if ( 'active' == $status ) new MP_Stat( 'u', 'active', -1 );
		}
		return $update;
	...
	public static function list_unsubscribed( $id ) 
	{
		return self::unsubscribed( $id, true );
	}
	...
	{
		return ( MailPress::is_email( $email ) && self::status_deleted != self::get_status_by_email( $email ) ) ; 
	}
...
/// USER ///
	...
	public static function insert( $email, $name, $args = array() )
	{
		...
		$defaults = array( 	'status'		=> 'waiting',
						...
						'created_IP'	=> trim( $REMOTE_ADDR ),
						'created_agent'	=> trim( strip_tags( $HTTP_USER_AGENT ) ),
...
/// MAIL URLs ///
	...
	private static function _get_url( $action, $key ){
 	...
 	public static function get_arch_url( $id, $key = 0 )
 	{
		return self::_get_url( 'arch', $key ) . "&id=$id";
 	}
