...
abstract class MP_import_importer_
{
	...
	function get_step()
	{
		return (int) filter_input( INPUT_GET, 'step', FILTER_CALLBACK,  array( 'options' => array( $this, 'filter_step' ) ) );
	}

	function filter_step( $step )
	{
		if ( !isset( $step ) || !is_numeric( $step ) ) return 0;
		return $step;
	}
...
// step 0
	...
	function greet() 
	{
		...
		$out = '';
		$out .= '<div><p>' . __( 'Howdy! Upload your file and we&#8217;ll import the emails and much more ... into this blog.', 'MailPress' ) . '<br />';
		$out .= __( 'Choose a file to upload, then click Upload file and import.', 'MailPress' ) . '</p>' . "\r\n";
		echo $out;

		wp_import_upload_form( add_query_arg( $args, MailPress_import ) );
