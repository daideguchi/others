...
abstract class MP_newsletter_processor_ extends MP_newsletter_
{
	...
	function already_processed()
	{
		...
		MP_Newsletter_processors::message_report( $this->newsletter, "old : {$this->old_lower_bound}, lower : {$this->lower_bound}, upper : {$this->upper_bound}", $this->trace, true );

		if ( max( $this->old_lower_bound, $this->lower_bound ) >=  $this->upper_bound )
		{
			MP_Newsletter_processors::message_report( false, "newsletter already processed : ( " . max( $this->old_lower_bound, $this->lower_bound ) . " >= {$this->upper_bound} ) ", $this->trace, true );
			return true;
		}
		...
		$query_posts = $this->newsletter[$this->args]['query_posts'] ?? array();
	...
	{
		$this->old_lower_bound = $this->options['end'] ?? $this->lower_bound;
	}
