...
class MP_Form_field
{
	...
	public static function insert( $Post_field ) 
	{
		$Post_defaults = array( 'id' => 0, 'form_id' => 0 );
		$Post_field = wp_parse_args( stripslashes_deep( $Post_field ), $Post_defaults );

		$Post_field['label'] 		= sanitize_text_field( $Post_field['label'] );
		$Post_field['description'] 	= sanitize_text_field( $Post_field['description'] );

		extract( $Post_field, EXTR_SKIP );
