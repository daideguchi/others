...
class MP_Mail extends MP_mail_
{
	...
	{
		if ( !isset( $this->_debug ) ) $this->_debug = ( defined( 'MP_DEBUG_LOG' ) );
...
////  Build it  ////
		...
		$this->row->id      = $this->args->id       = $this->args->id       ?? MP_Mail::get_id( 'MP_Mail::start' );
		$this->row->list_id = $this->args->_list_id = $this->args->_list_id ?? 'MailPress.0';

	//# charset #//
		$this->row->charset = $this->args->charset ?? get_option( 'blog_charset' );
	...
	//# subject #//
		$this->row->subject 	= $this->args->subject ?? false;
		if ( isset( $this->args->Template ) ) 
		{
			$theme = $this->args->Theme ?? $this->theme->themes[$this->theme->current_theme]['Stylesheet'];
	...
	//# html #//
		$this->row->html		= $this->build_mail_content( 'html', 'MailPress_the_content' );
	...
	//# only one recipient #//
		if ( 1 == $this->mail->recipients_count )
		{
			...
			$this->row->replacements[$toemail] = array_merge( $this->row->replacements[$toemail] ?? array(), $this->mail->replacements );
			...
			if ( isset( $this->row->replacements[$toemail]['{{_confkey}}'] ) ) $this->row->confkey    = $this->row->replacements[$toemail]['{{_confkey}}'];
...
////  Build it  ////
 ...
 //# mail replacements #//
	...
	function get_mail_replacements()
	{
		...
		$mail_main_id = $this->args->main_id ?? 0;
 ...
 //# recipients & replacements #//
	...
	function manage_recipients()
	{
		...
		$is_email = MailPress::is_email( $this->args->toemail ?? false );
	...
	function get_recipients( $query )
	{
		...
		if ( $mp_users )
		{
			...
			{
				if ( !MailPress::is_email( $mp_user->email ) )
				{
					$this->trace->log( 'MAILPRESS [WARNING] - :::: this recipient ' . $mp_user->email . ' discarded due to applicable RFC ::::'  );
					continue;
				}
...
if ( $this->_debug ) $eml = $this->message->toString(); file_put_contents( MP_UPL_ABSPATH . $this->row->id . '.eml', $eml );
		}
		catch ( Exception $e ) 
		{
...
if ( $this->_debug ) $eml = $this->message->toString(); file_put_contents( MP_UPL_ABSPATH . $this->row->id . '.eml', $eml );
	...
	//# Swift connection #//
		...
		if ( isset( $this->mail->mailpress_batch_spool_send ) && $this->mail->mailpress_batch_spool_send )
															new MP_Swift_Connection_spool();
		...
		try 
		{
			...
			$this->swift = new Swift_Mailer( $conn );
		}
		catch ( Exception $e ) 
		{
			if ( !( $e instanceof Swift_SwiftException ) ) throw $e;
...
//# Swiftmailer logging #//
if ( $this->_debug ) $this->trace->log( 'SWIFTMAILER [LOGGING] - ' . $logger->dump() . "\n\n" );
...
if ( $this->_debug ) $this->trace->log( 'SWIFTMAILER [LOGGING] - ' . $logger->dump() . "\n\n" );
	...
	//# message #//
		...
		$this->message->setEncoder( Swift_DependencyContainer::getInstance()->lookup('mime.qpcontentencoder') );
	...
	//# headers from wp_mail ? #//
		if ( isset( $this->mail->recipients_count ) && ( 1 == $this->mail->recipients_count ) )
		{
			$meta = MP_Mail_meta::get( $this->row->id, '_MailPress_accepted_headers' );
			if ( $meta )
			{
				$hfunc = array( 'cc' => 'addCc', 'bcc' => 'addBcc', 'reply_to' => 'addReplyTo' );
				foreach( $meta as $k => $v )
				{
					if ( is_array( $v ) )
					{
						...
						{
							case 'X' :
								$_headers = $this->message->getHeaders();
								foreach( $v as $vv ) { foreach( $vv as $name => $content ) { $_headers->addTextHeader( $name, $content ); } }
							break;
							case 'return_path' :
								$this->message->setReturnPath( $v[0] );
							break;
							case 'cc' :
							case 'bcc' :
							case 'reply_to' :
								if ( !isset( $hfunc[$k] ) ) break;
								$func = $hfunc[$k];
								foreach( $v as $toemail => $toname )
								{
									try
									{
										$this->message->$func( $toemail, $toname );
									}
									catch ( Exception $e ) 
									{
										if ( !( $e instanceof Swift_SwiftException ) ) throw $e;
										$this->trace->log( 'SWIFTMAILER [WARNING] - ' . "{$hfunc[$k]} - Recipient do not comply RFC rules ( discarded ) :\n\n" . $e->getMessage() . "\n\n" );	
									}
								}
							break;
						}
	...
	//# signer #//
		...
		{
			$Swift_Signer_type = apply_filters( 'MailPress_Swift_Signer_type', false );

			if ( $Swift_Signer_type )
			{
				$signer = apply_filters( "MailPress_Swift_Signer_{$Swift_Signer_type}", false );
				if ( $signer )
				{
					$this->message->attachSigner( $signer );
					$this->trace->log( '!            ! [notice] MailPress - Signer found : ' . $Swift_Signer_type, $this->trace->levels[512] );
				}
			}
		}

	//# MailPress_swift_message_built #//
		do_action( 'MailPress_swift_message_built', $this->message, $this->row );
	}
	...
	function process_img( $html, $path, $dest = 'mail' )
	{
		...
		$out = preg_match_all( '/<img[^>]*>/Ui', $html, $imgtags, PREG_SET_ORDER ); // all img tag

		if ( empty( $imgtags ) ) return $html;

		foreach ( $imgtags as $imgtag )
		{
			$out = preg_match_all( '/src=[\'"]([^\'"]+)[\'"]/Ui', $imgtag[0], $src, PREG_SET_ORDER ); // for src attribute
		...
		foreach ( $matches as $match )
		{
			...
			foreach ( $masks as $mask )
			{
				...
				if ( is_file( ABSPATH . $file ) ) 
				{
					...
					if ( isset( $this->trace ) ) if ( 'mail' == $dest ) $this->trace->log( '!            ! [notice] MailPress - Image found : ' . $file, $this->trace->levels[512] );
				}
				if ( $f ) break;
...
/// DISPLAYING MAILS ///
	...
	function get_replacements( $id, $main_id = false, $mp_user_id = false )
	{
		...
		if ( MailPress::is_email( $mail->toemail ) ) return $mail_r;
	...
	function view( $args )
	{
		...
		if ( !in_array( $x->status, $y ) )
		{
			...
			$x->{$type} 		= $this->build_mail_content( $type, ( 'html' == $type ) ? 'MailPress_the_content' : false );
		...
		else
		{
			...
			if ( is_array( $y ) )
			{
				...
				foreach ( $y as $k => $v )
				{
					...
					$opt = ( is_numeric( $k ) ) ? __( 'anonymized', 'MailPress' ) : $k;
					$return .= "<option$s>$opt</option>";
...
/* ajax */
	public static function add_object()
	{
		MP_WP_List::add_object( self::object_type );
	}

	public static function dim_object() 
	{
		MP_WP_List::dim_object( self::object_type );
	}

	public static function delete_object() 
	{
		MP_WP_List::delete_object( self::object_type );
	}
}