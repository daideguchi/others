...
class MP_Form
{
	...
	public static function insert( $Post_form ) 
	{
		$Post_defaults = array( 'id' => 0 );
		$Post_form = wp_parse_args( stripslashes_deep( $Post_form ), $Post_defaults );

		$Post_form['label'] 		= sanitize_text_field( $Post_form['label'] );
		$Post_form['description']	= sanitize_text_field( $Post_form['description'] );

		extract( $Post_form, EXTR_SKIP );
...
//// the form /building/resetting/controlling 
	...
	public static function form( $id )
	{
		...
		$submitted = ( ( isset( $post_[self::prefix][$id] ) ) );
...
//// Getting form
	public static function get_form( $form, $fields, $form_templates, $submitted = false, $processed = false )
	{
		...
		if ( $submitted )
		{
			$status = ( $processed ) ? 'ok' : 'ko';
			$message[ $status ] = $form->settings['message'][ $status ];
		}
