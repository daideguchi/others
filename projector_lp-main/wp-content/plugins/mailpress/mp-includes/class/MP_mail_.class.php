...
abstract class MP_mail_ extends MP_WP_db_connect_
{
	...
	function get_the_content( $more_link_text = null, $stripteaser = 0 )
	{
		...
		if ( isset( $this->args->newsletter ) && $this->args->newsletter )
		{
			...
			return ( $this->build->plaintext ) ? $this->html2txt( apply_filters( 'MailPress_the_content', $content ) ) : $content;
		}

		if ( isset( $this->args->content ) && !empty( $this->args->content ) )
			$content = ( $this->build->plaintext ) ? $this->html2txt( apply_filters( 'MailPress_the_content', $this->args->content ) ) : $this->args->content;
