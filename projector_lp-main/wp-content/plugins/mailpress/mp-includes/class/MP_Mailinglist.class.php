...
class MP_Mailinglist
{
	...
	public static function insert( $term_arr, $wp_error = false ) 
	{
		...
		$term_arr['name'] 		= sanitize_text_field( $term_arr['name'] );
		$term_arr['description'] 	= sanitize_text_field( $term_arr['description'] );
...
////  Layouts  ////
	...
	public static function dropdown( $args = '' ) 
	{
		...
		if ( ! empty( $mailinglists ) ) 
		{
			...
			$out = '<select name="' . $htmlname . '" ' . $htmlid . ' class="' . $class . '" ' . $tab_index_attribute . '>';

			if ( $show_option_all ) 
				$out .= '<option value="0">' . $show_option_all . '</option>';

			if ( $show_option_none ) 
				$out .= '<option value="-1">' . $show_option_none . '</option>';

			if ( $hierarchical )	$depth = $r['depth'];  	// Walk the full depth.
			else				$depth = -1; 			// Flat.

			$out .= self::walk_dropdown_tree( $mailinglists, $depth, $r );
			$out .= '</select>' . "\n";
		}
	...
	public static function walk_array() 
	{
		...
		$args = func_get_args();
		return unserialize( call_user_func_array( array( &$walker, 'walk' ), $args ) );
	}
